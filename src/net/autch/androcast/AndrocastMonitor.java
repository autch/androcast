...
public class AndrocastMonitor extends SwingWorker<Object, Boolean> {
            ...
            while(!isCancelled()) {
                ...
                synchronized(this) {
	                if(resize) {
	                    ...
	                    if(landscape) {
	                    	raw_width = height;	raw_height = width;
	                    }
	                	img_width = (int)(raw_width * zoom);
	                	img_height = (int)(raw_height * zoom);

	                	// reconstruct image buffer
                        image = new BufferedImage(raw_width, raw_height, BufferedImage.TYPE_INT_ARGB);
    ...
    public synchronized void render(Graphics g) {
    	g.drawImage(image, 0, 0, img_width, img_height, 
				0, 0, raw_width, raw_height, null);
    }
    
    private static void transformRawImageP(BufferedImage image, RawImage rawImage) {
    ...
    private static void transformRawImageL(BufferedImage image, RawImage rawImage) {
        // convert raw data to an Image
        byte[] buffer = rawImage.data;
        int index = 0;
        for (int y = 0 ; y < rawImage.height ; y++) {
